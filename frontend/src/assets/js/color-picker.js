function assignColors(flashcards) {
    const colors = ["#F94144", "#F3722C", "#F8961E", "#F9844A", "#F9C74F", "#90BE6D", "#43AA8B", "#4D908E", "#577590", "#277DA1"];
    let lastColorIndex = -1; // Track the index of the last color used
  
    flashcards.forEach(flashcard => {
      let colorIndex;
      do {
        // Select a random index from the colors array
        colorIndex = Math.floor(Math.random() * colors.length);
      } while (colorIndex === lastColorIndex); // Ensure it's not the same as the last one
  
      // Assign the color and update lastColorIndex
      flashcard.color = colors[colorIndex];
      lastColorIndex = colorIndex;
    });
  
    return flashcards;
  }

export default assignColors;